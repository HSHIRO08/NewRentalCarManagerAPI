...
public class BookingService : IBookingService
{
    ...
    public BookingService(IUnitOfWork uow, IEmailService emailService)
    {
        _uow = uow;
    ...
    public async Task<BookingDto> QuickCreateAsync(Guid renterId, QuickCreateBookingDto dto)
    {
        ...
        // Save immediately so the entity is persisted
    ...
    public async Task<BookingDto?> PayBookingAsync(Guid bookingId, Guid payerId)
    {
        ...
        if (!string.IsNullOrEmpty(renterEmail))
        {
            var emailData = new BookingEmailData
            {
                ...
                CarName = carName,
                RentalStart = booking.RentalStart.ToString("dd/MM/yyyy"),
                RentalEnd = booking.RentalEnd.ToString("dd/MM/yyyy"),
                TotalPrice = $"{booking.TotalPriceVnd:N0} đ",
                PaidAt = paidAt
            };

            var sent = await _emailService.SendBookingConfirmationAsync(renterEmail, renterName, emailData);
            ...
            var log = new NotificationLog
            {
                ...
                Recipient = renterEmail,
                Payload = $"Booking #{booking.Id} payment confirmed. Total: {booking.TotalPriceVnd:N0} đ",
                Status = sent ? NotifStatus.Sent : NotifStatus.Failed,
                SentAt = sent ? paidAt : null,
                ErrorMsg = sent ? null : "SMTP send failed",
                CreatedAt = paidAt
            };
            await _uow.NotificationLogs.AddAsync(log);
        }
        else
        {
            emailStatus = "Skipped";
